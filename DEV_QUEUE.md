...
<!-- ▼ 2026-08-14 ユーザー指示。監査由来ではなく直接発注なので、新規コンテンツだが TODO に置く -->
...
- [ ] (P2) 追加する敵イラストの仕様を詰める | artist | 上の「敵の種類を増やす」で追加した敵に対し、**どんな絵が要るかの発注書を書く**（`docs/` に md で出力）。1体につき: 何のモチーフか・pre（穢れ）/post（浄化）の差・シルエットで既存敵と区別が付くか・色。**画像生成そのものはこの環境では実行できない**（`gen-asset` は OpenAI APIキー `.openai_key.txt` を要求し、クラウド実行環境には存在しない）。**生成はユーザーのローカル環境で行うので、ここでは仕様書の作成までで完了とすること**
