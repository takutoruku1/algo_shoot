...
> **この記事の範囲**: ハブでこはるの投稿を選んでから、こはるを浄化してハブへ戻り、帰還の投稿・炎上・返信・再訪の小話までの流れ。こはるの人物像は → [こはる](../04_キャラクター/05_こはる.md)。ボス戦の仕組み全般は → [ボス戦](../05_ゲーム仕様/06_ボス戦.md)。面の順は あかり → こはる → レイ で、この記事が二面目である。
