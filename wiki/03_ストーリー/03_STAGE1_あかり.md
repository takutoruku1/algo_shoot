...
> **この記事の範囲**: ハブであかりの投稿を選んでから、あかりを浄化してハブへ戻り、帰還の投稿・返信・再訪の小話までの流れ。あかりの人物像は → [あかり](../04_キャラクター/04_あかり.md)。ボス戦の仕組み全般は → [ボス戦](../05_ゲーム仕様/06_ボス戦.md)。面の順は あかり → こはる → レイ で、この記事が一面目である。この章が流れるのは結び手（ミナ）で潜ったときで、他のアカウントではそのキャラ専用の物語になる（→ [09 キャラ別ストーリー](09_キャラ別ストーリー.md)）。
...
カードを選ぶと投稿詳細が開き、潜り方の段ごとにミナの一言が添えられる（→ [難易度](../05_ゲーム仕様/08_難易度.md)）。決定するとダイブが始まり、画面に「STAGE 1 START」の帯が出る。
