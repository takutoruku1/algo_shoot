...
#### 未確定・実装側の宿題（この枠を採用するとき）

- E5b は**ループ不要＝1 周で終わって無音**という扱いが要るが、既存の `Audio.Music()`（`src/Audio.cs:325-360`）は
  **ループ前提でクロスフェード再生する**経路しかない。台本の「『…………。』で完全停止（無音）」を実装するには
  **再生制御の追加が要る**（未設計・未実装）。
- ローダーは `LoadBgmMenu()`（`src/Audio.cs:971`）と同型で 1 本追加、フォールバックは `BgmMenu`。
- 甘茶／PeriTune ともクレジットは任意だが、**採用したら `config/credits.ini` の `[音楽]` に記載する**
  （既存曲と同じ運用。任意でも書く方針で通っている）。
