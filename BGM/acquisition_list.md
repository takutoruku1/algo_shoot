...
| 2026-09-08 | **bgm_epilogue_walk**（新規11本目） | 夕べの星 | 甘茶の音楽工房 | https://amachamusic.chagasi.com/music_yuubenohoshi.html （規約 https://amachamusic.chagasi.com/terms.html ） | ○/○/○ | 任意（記載済） | エピローグ E5b 専用・**唯一のループしない曲（import loop=false）**。原曲 157.4s → **0..74.65s・-3.7dB**・頭40ms/尻250msフェード。切り出し理由は下記 §6.1 |
...
### 6.1 bgm_epilogue_walk「夕べの星」— 切り出しの根拠（2026-09-08 composer）

**採用**: ユーザー決定（2026-09-08）。候補5本は `BGM/candidates.md` ⑬ に全て残してある。

**ライセンス確認（2026-09-08・規約原文 https://amachamusic.chagasi.com/terms.html を当日取得）**
- 商用利用可（「商用利用、個人利用問わず利用できます」）／ゲーム組込可（「ゲームなど、何かのBGMとして」）
- 改変可（Q&A 原文「テンポやキーの変更、音の付加、フェードイン・フェードアウトなど自由に加工できます」）
- 形式変換可（Q&A 原文「ご都合の良いファイル名、ファイル形式に変更してお使い下さい」＝mp3→ogg が明示的に可）
- クレジット**任意**（「サイト名、作曲者名、URLのいずれか一つでＯＫ」）→ 任意だが `config/credits.ini` に記載
- 禁止事項に該当なし: 音楽単体の販売・2次配布（本作は pck 埋込＝該当せず）／作曲者を偽る行為／
  JASRAC 等への登録／**YouTube Content ID への登録**（配布時に注意。ゲーム本体の配布には無関係）
- 取得方法: 配布ページの『ダウンロード』リンク（`mp3/yuubenohoshi.mp3`）を保存＝ページ導線どおり。
  規約の禁じる「直リンクでの使用」は他サイトからのホットリンクを指し、DL 自体はこの手順が正規。
- マスターは `BGM/yuubenohoshi.mp3`（原曲そのまま・157.36s / 128kbps。export からは `BGM/*` で除外）

**曲の構造解析（110.0 BPM・1小節 2.182s・A メジャー。実測）**
```
0.5s  b0  イントロ（オルゴール単独・低音量）
4.8s  b2  A セクション（主旋律）        ← ここから本題
39.7s b18 B セクション（盛り上がり・最大エネルギー 0.99）
74.65s b34 A セクションが再開（＝1周目の終わり）  ★ここで切った
109.6s b50 B 再現 → 144s〜 コーダ＋フェードアウト
```
**切り出し = 0.00 〜 74.65 秒**（74.65s）。理由:
1. **冒頭から始まる**（イントロを含むので、いきなり展開の途中から始まらない＝指定を満たす）。
2. **終端がフェードアウトではなく曲想の切れ目**。74.65s は「1周目が終わり A が再開する直前」で、
   実測で 74.45〜74.60s が余韻の谷（-24dB）、74.69s が次フレーズのアタック（-8.6dB）。
   谷で切っているので、**止めても曲を裁断した音がしない**＝台本の「…………。」での完全停止と噛み合う。
3. 70〜90 秒の指定帯に収まる（74.65s）。
- 尻 250ms のフェードは「切断」ではなく**残響の自然な減衰**のため（原曲の余韻を殺さない範囲）。

**ゲイン: -3.7dB → -17.6 LUFS**（原曲 -14.0 LUFS / true peak -1.6dBFS → 加工後 true peak -5.8dBFS）。
既存曲は「ピーク -3dB 帯」で揃えてきたが**ラウドネスは -9.4〜-19.3 LUFS とばらついている**ため、
この曲は**ピークではなくラウドネスを、直後に鳴る `bgm_menu_mina`（-17.7 LUFS）へ合わせた**。
E5b の無音 → E6 で BgmMenu が戻る繋ぎで段差を出さないことを優先した判断（§0 のレシピからの意図的な逸脱）。

**再生系での扱い（既存曲と違う点。触るときは注意）**
- `.import` は **`loop=false`**（既存10曲は `loop=true`）。ループしない曲なので焼き込みを外している。
- `Audio.MusicTargetDb()` で **0dB 扱いに除外**した（`BgmMenu`・`BgmFinalResolve` と同じ扱い）。
  除外しないと実音源として `StageBgmRealDb = -10dB` が乗り、BgmMenu より 10dB 沈んで繋ぎが破綻する。
  音源側を BgmMenu と同じラウドネスへ寄せてあるので、ここは 0dB が正。

**E5b の実尺と曲尺の関係（実測。`--audiolog` で計測）**
| 読む速さ（行を読み終えてから送るまで） | 「…………。」時点の曲位置 | E5b 全体（見上げ10行＋歩行8行） |
|---|---|---|
| 0.8 秒（早送り気味） | 6.0s | 14.6s |
| 2.5 秒（標準的） | 17.9s | 45s |
| 4.0 秒（ゆっくり） | 28.3s | 68s |

→ **曲は最後まで鳴らない**（停止は曲尾ではなく台詞に同期するので、これは仕様どおり）。
どの速さでも**イントロ＋A セクション（0〜29s）しか鳴らず、B セクションの盛り上がり（39.7s〜・
エネルギー 0.89〜0.99）には到達しない**＝結果として「泣かせにかからない」条件に有利に働いている。
台本の「実測 70〜90 秒」は過大見積もりで、**実装の実尺は 15〜68 秒**（既定 48 字/秒・1行1ページ）。
74.65s の尺は「どんなにゆっくり読んでも曲が尽きない」保険として機能しており、短く詰める必要はない。

---

## 7. 非戦闘画面の個別曲 6本（2026-09-14 導入・composer）

**背景**: ユーザー指示「ところどころBGMが入ってないから、全部の画面で違うBGMが流れるように、
適したBGMをネットから取得してきて、入れ込んで。**必ず商用利用できるものが条件**」。
実装から台帳を作り直した結果（§7.1）、**戦闘は既に1画面1曲で足りており、
非戦闘12画面が `bgm_menu_mina` 1本という一点**が問題だった。そこを6枠に割った。

**選定・設計の詳細（候補と選定理由・対抗馬）は `BGM/candidates.md` ⑭〜⑲**。

### 7.1 取得記録

加工は全て既存レシピ（トリム → ゲイン → 頭40ms/尻120ms極小フェード → ogg q6 → import loop=true）。
**ゲインはピーク基準ではなくラウドネス基準**（-17.7 LUFS ＝ `bgm_menu_mina` に合わせた。理由は §7.2）。
マスターは `BGM/<slug>.mp3`（export除外）。曲別の加工値は `src/Audio.cs` の各 `LoadBgm*()` コメントが正。

| 日付 | スロット | 曲名 | サイト/作者 | 規約URL | 商用/改変/組込 | クレジット義務 | 備考 |
|---|---|---|---|---|---|---|---|
| 2026-09-14 | bgm_title | 見上げた空 | 甘茶の音楽工房 | https://amachamusic.chagasi.com/music_miagetasora.html | ○/○/○ | 任意（記載済） | 原曲 170.12s → **0..161.6s・-5.4dB**。末尾フェード開始(161.9s)の直前で切る |
| 2026-09-14 | bgm_prologue | オーヴⅡ | 甘茶の音楽工房 | https://amachamusic.chagasi.com/music_orb2.html | ○/○/○ | 任意（記載済） | 原曲 155.24s → **0..148.0s・-3.1dB**。減衰開始(148.3s)の直前。LRA 2.0 の平坦曲 |
| 2026-09-14 | bgm_shop | シンプルスタイル | 甘茶の音楽工房 | https://amachamusic.chagasi.com/music_simplestyle.html | ○/○/○ | 任意（記載済） | 原曲 125.02s → **0..111.9s・-3.9dB**。減衰開始(112.2s)の直前 |
| 2026-09-14 | bgm_training | プルキンエ・フェノミナン | 甘茶の音楽工房 | https://amachamusic.chagasi.com/music_purkinjephenomenon.html | ○/○/○ | 任意（記載済） | 原曲 119.01s → **0..113.0s・-1.8dB**。減衰開始(113.3s)の直前 |
| 2026-09-14 | bgm_credits | 帰り道 | 甘茶の音楽工房 | https://amachamusic.chagasi.com/music_kaerimichi.html | ○/○/○ | 任意（記載済） | 原曲 166.52s → **0..150.8s・-5.6dB**。長い自然減衰の入口で切る（最長枠＝スクロール中に尽きない） |
| 2026-09-14 | bgm_gameover | 悲しみのテクスチャⅠ | 甘茶の音楽工房 | https://amachamusic.chagasi.com/music_kanashiminotexture1.html | ○/○/○ | 任意（記載済） | 原曲 120.20s → **0..103.5s・-5.1dB**。減衰開始(103.8s)の直前 |

### 7.2 ライセンス確認（**規約原文を 2026-09-14 当日に取得して確認**）

規約ページ https://amachamusic.chagasi.com/terms.html を当日 curl で取得し、原文で以下を確認した
（2026-09-08「夕べの星」導入時と**同じ条件が維持されている**ことを再確認した形）。

- **商用利用可** — 原文「公開中の音楽素材は、商用利用、個人利用問わず利用できます。」
- **ゲーム組込可** — 原文「音楽素材は、ウェブ、映像、ゲーム、ラジオなど、何かのBGMとしてお使い下さい。」
- **改変可（必須条件）** — Q&A 原文「Q:加工して使用しても良いですか？ A:問題ありません。テンポやキーの変更、
  音の付加、フェードイン・フェードアウトなど自由に加工できます。」＝本パイプラインのトリム／ゲイン／
  フェード／ogg 再エンコードは全て明示的に許諾の範囲内
- **形式変換可** — Q&A 原文「Q:mp3のファイル名やファイル形式を変更しても良いですか？ A:問題ありません。」
- **クレジット任意** — 原文「著作権表示は必須ではありませんが、ご都合のよい箇所にリンクorクレジット
  していただけると嬉しいです。表記内容はサイト名、作曲者名、URLのいずれか一つでＯＫです。」
  → **任意だが `config/credits.ini` [音楽] に6曲とも記載**（既存曲と同じ運用）
- **禁止事項に該当しないこと**を個別確認:
  - 「音楽だけを販売したり、2次配布することは禁止」→ 本作は pck 埋込＝音源を単体で取り出す形ではない（該当せず）
  - 「作曲者を偽る行為」→ クレジット記載により該当せず
  - 「JASRAC など著作権管理団体に登録」→ 行わない
  - 「YouTube の Content ID に登録」→ 行わない。**配布・実況対応時の注意事項としてここに明記しておく**
  - 「音楽素材の直リンクでの使用は禁止」→ 直リンクは**他サイトからのホットリンク**を指す。
    取得は配布ページのダウンロード導線（`mp3/<slug>.mp3`）を保存＝規約の想定どおりの正規手順
- **使用報告・許諾書**: Q&A 原文「使用報告は必須ではありません」「使用許諾書の発行はしておりません」
  → 報告義務なし。許諾書が要る用途では使えないという制約は認識済み
- **免責**: 「音楽素材やコンテンツは予告無しに変更・削除されることがあります」
  → **マスターを `BGM/` に保存済み**なので、配布ページが消えても本作の利用には影響しない

### 7.3 ラウドネスを既存曲へ揃えた根拠（実測）

既存曲は「ピーク -3dB 帯」で揃えてきたが、**ラウドネスは -9.4〜-19.3 LUFS と大きくばらついている**。
今回の6曲はすべて**非戦闘＝メニュー一族**で、`bgm_menu_mina`（ハブ）と行き来するので、
ピークではなく**ラウドネスを `bgm_menu_mina` の -17.7 LUFS に合わせた**（「夕べの星」-17.6 と同じ判断）。

実測値（加工後・ffmpeg ebur128）:

| スロット | Integrated | LRA | True Peak | 尺 | 基準との差 |
|---|---|---|---|---|---|
| （基準）bgm_menu_mina | **-17.7** | 4.4 | -3.1 | 92.0s | — |
| （前例）bgm_epilogue_walk | -17.6 | 3.1 | -5.8 | 74.7s | +0.1 |
| bgm_title | -17.6 | 6.1 | -6.7 | 161.6s | **+0.1** |
| bgm_prologue | -17.7 | 2.0 | -4.5 | 148.0s | **±0.0** |
| bgm_shop | -17.7 | 1.5 | -4.2 | 111.9s | **±0.0** |
| bgm_training | -17.7 | 2.9 | -3.5 | 113.0s | **±0.0** |
| bgm_credits | -17.7 | 2.9 | -5.5 | 150.8s | **±0.0** |
| bgm_gameover | -17.7 | 3.4 | -5.1 | 103.5s | **±0.0** |

参考・**今回触っていない**既存曲の実測（戦闘曲が大きいのは従来どおり。`MusicTargetDb()` の
`StageBgmRealDb = -10dB` が再生時に乗るため、実際の出音では揃う）:
`bgm_boss_mina` -9.4 / `bgm_boss_akari` -11.1 / `bgm_boss_koharu` -11.2 / `bgm_boss_hikage` -13.6 /
`bgm_boss_rei` -14.1 / `bgm_final_resolve` -14.3 / `bgm_stage_rei` -14.5 / `bgm_stage_akari` -16.9 /
`bgm_stage_koharu` -18.7 / `bgm_stage_w0` -19.3
