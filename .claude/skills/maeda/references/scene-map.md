...
| STAGE1 レイ（道中） | `src/StageRei.cs` | `Intro`/`BossIntro`/`Clear` の配列 | `(who,text,face)[]` |
| STAGE1 レイ（改心） | `src/BossRei.cs` | `Lines[]` | `(who,text,face)[]` |
| STAGE2 あかり（道中） | `src/StageAkari.cs` | 同上 | `(who,text,face)[]` |
| STAGE2 あかり（改心） | `src/BossAkari.cs` | `Lines[]`（ナレ=記憶フラッシュ） | `(who,text,face)[]` |
| STAGE3 こはる（道中） | `src/StageKoharu.cs` | 同上 | `(who,text,face)[]` |
| STAGE3 こはる（改心） | `src/BossKoharu.cs` | `Lines[]` | `(who,text,face)[]` |
