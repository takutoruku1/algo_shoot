...
public partial class Final : Node2D
{
    ...
    public override void _Ready()
    {
        ...
        // 感動テコ入れ（docs/20260922/感動テコ入れ_改稿記録_2026-09-22.md ビート1）
        //   Prologue P2「わたくしの心拍、七十二だそうです。……心臓は、ありませんが。」（起動時の軽口）を
        //   ここで一度「途切れた」と申告させる＝次行「まだ、いらっしゃいますか」の孤独を集計の語彙で先に置く。
        //   答えは【初】が届いた後の「心拍、七十二。——あの日と、同じです。」（ApplyFinalChoice）。
        T("ミナ", "……心拍の集計が、途切れています。……回線が、細いので。");
    ...
    private void ApplyFinalChoice(int sel)
    {
        ...
        // 感動テコ入れ（docs/20260922/感動テコ入れ_改稿記録_2026-09-22.md ビート2）
        //   ①【初】が「最初の日に散った言葉」だと、ミナが観測として認識する一行。なぜ戻ったかは言わない
        //     （08 の指定どおり）。ただし「どの言葉だったか」を本人が言わないと、P2 で散らした言葉を
        //     覚えていないプレイヤーには伏線が伏線として届かない＝回収の最低条件だけを置く。
        //     【初】が空（旧セーブ・ボス直行）でフォールバック語のときは嘘になるので出さない。
        //   ②「心拍、七十二」＝P2 の軽口の意味反転。直前の「途切れています」→「まだ、いらっしゃいますか」の答えを、
        //     感情語ではなく集計の数字で返す。何の心拍かは言わない（ミナのでも、回線の向こうのでも読める）。
        if (!string.IsNullOrEmpty(_game?.FirstScattered))
            after.Add(new() { Who = "ミナ", Text = "……最初の日に、散った言葉ですね。" });
        after.Add(new() { Who = "ミナ", Text = "……心拍、七十二。——あの日と、同じです。" });
        // 最後の軽口。送信文字列の実数だけを差し込む観測（人格の断定は置かない）。
        after.Add(new() { Who = "ミナ", Text = $"……{word.Length}文字。……ふふ。相変わらず、短いですね。" });
        after.Add(new() { Who = "ミナ", Text = "今日は、もう、休ませてください。……戻ったら、空の話を、したいです。" });
        after.Add(new() { Who = "地",  Text = "——それから、わたくしは。帰るほうへ、自分で泳いでいきました。" });
