...
public partial class MinaPhaseScene : Node2D
{
    ...
    private static Line[] Dialogue(Job job, int phase)
    {
        string[] replies = (job, phase) switch
        {
            ...
            // 結び手（＝あなた／ご主人様）の返し。感動テコ入れ（docs/20260922/感動テコ入れ_改稿記録_2026-09-22.md ビート4）
            //   旧稿は「あなた」が句点つきの長文で喋っていた。本作の「あなた」は全編、短い下書きしか送らない
            //   （Hub「ご主人様の言葉には、句点が、ありません」／道中の選択肢も全部 2〜7 文字）ので、
            //   ここだけ人格が変わって見えた。短く・句点なし・言い切らない形へ戻す＝ミナの「助けて、ください」に
            //   長い慰めを返さない（言わせない）。他ジョブ（Melee/Heal/Magic）の腕はそのまま。
            (_, 1) => new[] {
                "ひとりで、持ってたんだ",
                "……聞きに来た",
            },
