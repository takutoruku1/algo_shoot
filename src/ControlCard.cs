...
public partial class ControlCard : Control
{
    ...
    private (string title, string lead, Color accent, Row[] rows) Spec(Topic t) => t switch
    {
        ...
        // キーボードは Shift の押しっぱなし（2026-09-26 作者決定：押した瞬間に最寄りへロック、離すと解除。
        //   F は「次の敵へ送り」として残る）。パッド RB／マウス左クリックは従来どおり押すたび送り。
        Topic.Lock => ("ロックオン", "近い敵から順に狙う。移動は少し遅くなる", UiKit.Purify, new[]
        {
            new Row("キーボード",     "Shift 長押し",             "押しているあいだ狙う。F で次の敵へ"),
        ...
        //   キーボードは Shift を離す（2026-09-26。旧 G は廃止）。
