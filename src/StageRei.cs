...
public partial class StageRei : Node
{
    ...
    private void Step_BossCameo(double delta)
    {
        ...
        if (!IsInstanceValid(_cameo) || _cameo.Finished)
        {
            ...
            if (CheckpointFlow.OnMidBossCleared(this, "rei", false, _cameo)) return;
            Advance();
        }
    }
    ...
    private void Step_Clear(double delta)
    {
        if (!_clearBannerShown)
        {
            ...
            if (noHit) game?.RecordNoHitClear("rei", game.Difficulty);
            Hud.ShowClearBanner("STAGE 3 CLEAR", _clearTime, rec.isBest, rec.prev, score, recScore.isBest, recScore.prev, noHit);
