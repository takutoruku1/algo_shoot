...
public partial class StageRei : Node
{
    ...
    private static readonly (int who, string text, string face)[] Intro =
    {
        ...
        (1, "……いってまいります。ご主人様。", MDoubt),   // 潜る前の儀式・三度目（光が薄いぶん、小さく。docs/20260922 ビート6）
    };
    ...
    //   （一面・二面には導入がある）。台本 07 に導入行が無かったという実装都合がそのまま残っていた形。
    ...
    private void Step_BossCameo(double delta)
    {
        if (!_stepStarted)
        {
            ...
            _cameo = new CameoBoss
            {
                ...
                {
                    DisplayName = "レイ", Handle = BossHandles.ReiCameo,
