...
// 移動(通常150 / 低速65 px/s)、連射(Pool経由・射撃方向(_facing)へ+360・上下2way)、被弾無敵点滅、TakeHit、Lives。
...
public partial class Player : Area2D
{
    ...
    // postTexPath＝浄化元(EnemySpec.PostTexPath)の改心後テクスチャ。省略/空ならFollower側でアンチくんにフォールバック。
    public bool AddFollower(Vector2 globalFromPos, string postTexPath = "")
    {
        _savedCount++;
        ...
        var f = new Follower { SlotOffset = FollowerSlots[_followers.Count], PostTexPath = postTexPath };
        AddChild(f);
    ...
    public bool DodgeReady => _dodgeCd <= 0f && _dodgeTimer <= 0f;   // クールダウンが明けて今すぐ回避できるか（HUD操作ガイドの点灯に使う）。TryDodge の実行可否ガード(1019行目)と同条件に揃える＝回避モーション中(_dodgeTimer>0)はまだ再回避できないためHUDも点灯させない。
    ...
    //   本当の問題は下流にある2点: ①Enemy.cs:550 `Mathf.Clamp(b.Damage, 1, 4)` がボス無防備窓の1ヒット
    //   ダメージを一律4で頭打ちにし、focus_1の前提(rapid_power_1→shot_power_2, 素の威力3)を満たした時点で
    //   ほぼ食い尽くされている。②Panel.cs:109 のインク削りは b.Damage を一切参照しない（Ink--のみ）ため、
